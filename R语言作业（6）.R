...
assign("mooday",weekdays(as.Date("1969/07/02")),an_environment)
an_environment[["pythag"]]
an_environment$root
get("mooday",an_environment)
...
list2env(a_list)
nested_enviroment <- new.env(parent = an_environment)
...
f <- function(x9){y9 <- 1
g <- function(y9){(x9+y9)/2}g(x9)}
f(sqrt(5))
f <- function(x8){y8 <- 1
g(x8)
}
g <- function(y8){(x8+y8)/2
}
f(sqrt(5))
h <- function(x10){
  x10*y10
...
h(9)
y11 <- 19
h2 <- function(x11){if(runif(1)>0.5)y11 <- 12
x11*y11}
replicate(10,h2(9))

}
an_environment <- multipiles_of_pi
an_environment[["two_pi"]] <- c(2*pi)
an_environment[["three_pi"]] <- c(3*pi)
an_environment[["four_pi"]] <- (4*pi)
